...
class Resumen(models.Model):
    ...
    no_grabadas = fields.Float("Total compras internas no gravadas", readonly=True)
    importacion_resumen = fields.Float("Total compras de importacion", readonly=True)
    alicuota_general = fields.Float("Total compras internas gravadas por alicuota general", readonly=True)
    alicuota_general_mas = fields.Float("Total compras internas gravadas por alicuota general mas adicional", readonly=True)
    alicuota_reducida = fields.Float("Total compras internas gravadas por alicuota reducida", readonly=True)
    lines_r = fields.Many2many(comodel_name="libro.compras", readonly=True)
    ...
    def update_resume(self):
        if self.f_inicio <= self.f_fin:
            ...
            data = self.env['libro.compras'].search([("fecha",">=",self.f_inicio), ("fecha","<=",self.f_fin)])
            total_compras = 0
            compras_exentas = 0
            afectas_base = 0
            afectas_impto = 0
            ...
            for item in data:
                ...
                compras_exentas += item['compras_exentas']
                ...
                if item['importacion'] == "si":
                    importacion += item['total_compras']
            ...
            self['no_grabadas'] = compras_exentas
    ...
    def generate_pdf(self):
        ...
        record = self.env['libro.compras'].search([("id","=",self.id)])
        record.unlink()
    ...
    def generate_xlsx(self):
        ...
        ws.write_merge(3,4,18,23,"Compras afectas(Debito Fiscal)",bold_center)
        ...
        "Regístro Número MH", "Nombre del comprador","Estado", "R.Social Rif","Total compras\n(incluye iva)","Compras exentas"]
        ...
        for item in self.lines_r:
            ...
            ws.write_merge(row,row,8,9,item['nombre_comprador'],value_style)
            ws.write_merge(row,row,10,11,item['estado'],value_style)
            ws.write_merge(row,row,12,13,item['r_s_rif'],value_style)
            ws.write_merge(row,row,14,15,item['total_compras'],value_style)
            ws.write_merge(row,row,16,17,item['compras_exentas'],value_style)
        ...
        ws.write_merge(row,row,14,15,total_compras,total_style)
        ws.write_merge(row,row,16,17,compras_exentas,total_style)
        ...
        row += 1
        ws.write_merge(row,row,0,5,"Total compras internas no gravadas: " + str(round(compras_exentas,2)),result_line)
        row += 1
        ws.write_merge(row,row,0,5,"Total compras de importación: " + str(round(total_compras,2)),result_line)
        row += 1
        ws.write_merge(row,row,0,5,"Total compras internas gravadas por alicuota general: " + str(round(alicuota_gen,2)),result_line)
        row += 1
        ws.write_merge(row,row,0,5,"Total compras internas gravadas por alicuota general mas adicional: " + str(round(alicuota_gen_mas,2)),result_line)
        row += 1
        ws.write_merge(row,row,0,5,"Total compras internas gravadas por alicuota reducida: " + str(round(alicuota_gen_reducida,2)),
        ...
        return{
            ...
            'web/content/?model=libro.resumen&field=report&download=true&id=%s&filename=%s'%(self.id,self.name),
            'target': 'new',
            }